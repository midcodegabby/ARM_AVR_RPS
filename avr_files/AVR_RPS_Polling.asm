...
;*	UPDATED FOR NON-FUNCTIONAL INTERRUPT CONTROLLER
;*
;*  	Rock Paper Scissors
;* 	Requirement:
...
;
;
;	THIS IS AN UPDATED FILE THAT WORKS WITHOUT USING INTERRUPTS
;
;
...
INIT:
	...
	sts		UCSR1A, mpr
	ldi		mpr, 0b00011000 ; Rx and Tx enable, and 8 bit frame
	sts		UCSR1B, mpr
...
MAIN:
	...
	brne	phase_1 		; skip phase_0 if we have already sent the start signal
...
phase_0:
	...
	; this section is for checking for a start input (PD7 has been inputted)

	; check mpr against 0b10110000:
	; if the x-bit is a zero in mpr (input found for the xth bit), then that means that the 
	; x-bit result stored in mpr after performing a logical AND with 0b10010000 will also be a zero,
	; meaning that an input has been found if its bit in mpr is a zero.
	andi	mpr, 0b10010000 ; perform the logical and between mpr and its mask
	; if mpr[x-bit] = 1, then that means there was no input for the xth bit

	; compare mpr to 0b000100000: checking for a start game input: 
	...
	cpi		mpr, 0b00010000	; Check for PD7 input

	; if z flag was not set: want to go back to main as there was no start input
	brne	phase_0			; no input; go back to phase_0
	
	; start the game if we did not branch
	rcall	start_game	
...
phase_1:
	...
	; check the RXC1 bit in the mpr register
	; if the RXC1 bit is set, then that means that there is data in the receive buffer
	sbrc	mpr, RXC1		; skip the next instruction if there is no data in the receive buffer

	rcall	USART_recv		; go to the function if there is data to be read

	; compare opp with sendready
	cpi		opp, SendReady
	brne	phase_1			; loop until opp is sendready: only break out of this loop once we have received the start signal

	; after these two loops, it is fair to assume that we can initiate the game start screen.
	; update gamephase to be 2
	; store a 1 to gamephase
	ldi		mpr, 2
	mov		gamephase, mpr

	; write the game start message to the LCD screen
	; clear the first and second line of the LCD screen 
	rcall	LCDClrLn1
	rcall	LCDClrLn2
...
phase_2:
	...
	; if mpr has only the 4th bit ON (is a zero) then skip the rcall.
	sbrs	mpr, 4	; skip the next instruction if there is a PD4 input

	rcall	change_hand ; change the hand only if Z flag was not set
	...
	; compare opp to SendReady
	cpi		opp, SendReady
	brne	phase_3			; loop until opp is different than its sendready signal then go to next section

	rjmp	phase_2		; repeat loop
